        ...
        with open(path, encoding="utf-8") as split_f:
            for line in split_f:
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    task_data["Categories"] = ['instruction_induction']
                    task_data["Reasoning"] = ['instruction_induction']
                    task_data["Positive Examples"] = []
                    task_data["Negative Examples"] = []
                    task_data["Input_language"] = ['English']
                    task_data["Output_language"] = ['English']
                    task_data["Instruction_language"] = ['English']
                    task_data["Domains"] = ['instruction_induction']

                    if "Instruction Source" in task_data:
                        task_data.pop("Instruction Source")
                    all_instances = task_data.pop("examples")
                    ...
                    for idx, instance in enumerate(instances):
                        ...
                        for pair in input_output.split('\n\n'):
                            if pair.startswith('Input:'):
                                input_ = pair.split('\n')[0].strip('Input: ')
                                output_ = pair.split('\n')[1].strip('Output: ')
                        _instance = {'input': input_, 'output': [output_]}
                        example["Instance"] = _instance
